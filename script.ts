...
const cleanedData: CleanedData[] = dataWithoutTestsAndDuplicates
    .map(entry => {
        return {
            ...
            workshops: getWorkshops(entry.privateBody.workshop1, entry.privateBody.workshop2, entry.privateBody.workshop3),
...
cleanedData.forEach(({
    ...
    if (catering[0] === 2) {
        aggr.catering["Nein, ich werde für meine Verpflegung selbst sorgen"] += people;
    }
    if (catering[0] === 9) {
